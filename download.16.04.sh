...
[[ $? -ne 0 ]] && { echo "Only supports Ubuntu 16.04 system"; exit 1; }

DISTRO=$(lsb_release -c -s)
[[ ${DISTRO} -ne "xenial" ]] && { echo "Only supports Ubuntu 16.04 system"; exit 1; }
...
[ $(id -u) != "0" ] && {
    ...
    ansi -n --bold --bg-yellow --black "The current account is not root，Please use root Account to execute installation script（Use the command：sudo -H -s Switch to root）"
} || {
    bash ./laravel-stack-deployer-ubuntu/16.04/install.sh
}
